...
async function starts() {
	...
	const sotoy = [
		   ...
		   client.groupRemove(from, [Kick]).catch((e) => {reply(`*ERROR:* ${e}`)}) 
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'installbot':
				case 'instalbot': //Jangan dibully stah
				    Test = await getBuffer(`https://i.ibb.co/vk0B2t5/Test.jpg`)
				    client.sendMessage(from, Test, image, { quoted: mek, caption: '*_Cara Membuat Bot Whatsapp Di Android_*\n-Termux (Playstore)\n-Script\n-Ram yang cukup\n-Kouta\n-Niat\n*_Command yg anda butuhkan_*\n$termux-setup-storage\n$pkg install.upgrade && pkg install.update\napt get.upgrade && apt get.upgrade\n$pkg install ffmpeg && pkg install.git && pkg install.nodejs && pkg install.bash && pkg install.node && pkg install.nodejs && pkg install.libwebp && pkg install.wget && pkg install.mc && pkg install.tesseract\n$git clone https://github.com/rizkiramadhan4617/client \n$ls\n$cd client\n$ls\n$bash install.sh \n$node index.js\nOr\n$npm start\n*_Jalankan bot_*\n-Pergi ke whatsapp\n-Klick setelan\n-Cari tulisan _Whatsapp web_ Klick\n-Tekan tautkan perangkat\n-Scan qr nya \n\n-----------------------------------\n```Selamat mencoba```\n*_Jangan bully aku stah_*\n\nCopyright _@RR018_'})
				    break
				case 'bug':
		...
		/*case 'renungan':
			...
			case 'offcgroup':
			case 'offcgrup':
		    client.sendMessage(from, '*RR-018 GROUP*\n\nLink : https://chat.whatsapp.com/BAHKpyOMGcA2m6pHcj920C',MessageType.text, { quoted: mek })
		    break
			default:
